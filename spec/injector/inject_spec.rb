...
RSpec.describe Injector::Inject do
  describe '.register' do
    it 'has to create a Injectable instance' do
      described_class.register(:service, -> { Class.new })

      expect(described_class.injectables.first.class).to be Injector::Injectable
    end
  ...
  describe '.find' do
    ...
    context 'when a injectable with informed name was registered' do
      ...
      it 'has to return Injectable instance' do
        expect(described_class.find(:service).class).to be Injector::Injectable
      end
    end
  end
end
